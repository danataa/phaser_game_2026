...
export default class Merchant extends Actor {
  ...
  public executePurchase(player: Player, slotIndex: number): void {
    ...
    if (slot.perk.tipo === "permanente") {
      ...
      this._assegnaPerkTemporaneo(player, slot.perk.id);
    }
  ...
  private _assegnaPerkTemporaneo(player: Player, perkId: string): void {
    if (perkId === "scatto") {
      const slot = player.equipPerk(new PerkDash(player, 1800, 110, 1200));
      console.log(`⚡ Perk temporaneo Scatto assegnato allo slot ${slot}`);
      return;
    }

    if (perkId === "area") {
      const slot = player.equipPerk(new PerkAOE(player, 120, 50, 2000));
      console.log(`⚡ Perk temporaneo Area assegnato allo slot ${slot}`);
      return;
    }

    console.log(`⚡ Perk temporaneo ${perkId} non mappato`);
  }
