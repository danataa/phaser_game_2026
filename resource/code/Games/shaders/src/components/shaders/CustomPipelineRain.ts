export default class CustomPipelineRain extends Phaser.Renderer.WebGL
...
for (float d = 0.0; d < 6.28318530718; d += 0.19634954004) {
       
            for(float i=1.0/8.0; i<=1.0; i+=1.0/8.0)
            {
            
                vec3 tex = texture2D( iChannel0, UV+n+vec2(cos(d),sin(d))*Radius*i).rgb;
        ...
        vec3 tex = texture2D( iChannel0, UV+n).rgb;
        c.y = clamp(c.y, 0.0, 1.);
